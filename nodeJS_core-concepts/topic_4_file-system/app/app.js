...
(async () => {
  ...
  const createFile = async (path) => {
    try {
      // we want to check whether or not we have already that file
      const existingFileHandle = await fs.open(path, 'r');
      existingFileHandle.close();

      // we already have that file...
      return console.log(`The file ${path} already exists❗️`);
    } catch (error) {
  ...
  const deleteFile = async (path) => {
    ...
    try {
      ...
      if (e.code === 'ENOENT') {
        console.log('⛔️ No file at this path to remove.')
      } else {
  ...
  const renameFile = async (oldPath, newPath) => {
    ...
    try {
      ...
      if (e.code === 'ENOENT') {
        console.log('⛔️ No file at this path to rename, or the destination doesn\'t exist!');
      } else {
...
let addedContent;
  ...
  const addToFile = async (path, content) => {
    ...
    try {
      ...
      console.log('✅ The was added successfully!');
    } catch (e) {
